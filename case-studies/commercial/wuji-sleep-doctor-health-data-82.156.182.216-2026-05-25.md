...
Port 8000 runs a FastAPI/LangGraph service identifying itself as "Sleep Doctor Service" with `chat_service: "wuji-langgraph"`. The backend serves a WeChat Mini Program. No authentication.
...
Six additional data endpoints follow the same pattern, keyed by hash or unionid:
...
The hash and unionid values required are user identifiers surfaced through the log exposure below.
...
Each log entry contains timestamp, method, endpoint, status code, duration, hash, username, request body, response body, and client IP. Of 7,610 total requests, 6,712 succeeded. Those successful records contain user hash and openid values drawn from request bodies, sleep data responses inline, and the IP address of every client that queried the service.

This is the bridge. The log supplies the identifiers. The data endpoints accept those identifiers. The chain is read-only and requires no credentials at either step.
...
The monitor layer also exposes two write operations:
...
The service runs as root. Deployment path is `/root/sleepDoctor/`. Any code execution surface, present or future, in this stack starts with root-level access.
...
This data is not clinical in the HIPAA sense. It is personally sensitive health behavior data tied to stable user identifiers under China's Personal Information Protection Law (PIPL). PIPL treats health data as a sensitive personal information category requiring explicit consent and heightened protection. The exposure covers real users of a production WeChat Mini Program.
...
The MCP and retrieval endpoints suggest a second layer of capability beyond the direct data endpoints:
...
These were not probed. Retrieval index contents are uncharacterized.

---

## Thesis Placement

The auth model here is structural, not a misconfiguration. The GET endpoint takes an openid in the URL. The POST endpoints take a hash in the body. The server enforces nothing. The WeChat openid is treated as both the identifier and the credential.

This is a pattern from the early mobile backend era: client-supplied user IDs as implicit authentication. It predates session tokens. It predates JWT. On a sleep health platform serving production users in 2026, under PIPL, it is a live exposure.

The log file compounds it. Logs are supposed to be an internal audit trail. Here they are a public directory of user identifiers and health responses, with a destructor endpoint that any external party can call.

7,610 requests. 6,712 successful. Real users. Production service.
