...
| [backup-snapshot-services-survey-2026-05.md](backup-snapshot-services-survey-2026-05.md) | Backup & Snapshot Services (Qdrant `/snapshots` cross-cut) | 16 of 663 unauth Qdrant hosts expose pre-built snapshot files | **2,512 snapshot files = 269 GB bulk-downloadable.** Headline: **crm-fast.pl** (Polish CRM SaaS) exposing daily leads/emails/WhatsApp/interactions/decisions snapshots; **gptplane.it siena.backup** (Italian RAG SaaS multi-tenant backup server) with 18 months of daily snapshots = 226 GB. The snapshot endpoint inherits API auth state — operators with mature daily-backup workflows are at *higher* risk than those without |
