...
| **Browser automation / agent backends** | **98 cross-cloud (snapshot, probe still accumulating)** | **100% unauth at the platform endpoint** (CDP, Selenium status, Browserless health). 26 raw-Chromium CDP hosts = direct browser-RCE-equivalent via WebSocket. **21-host fleet at HeadlessChrome 121.0.6167.85** = single Browserless template mass-deployed (mirrors LLM-Gateway 1,829-host canned-response pattern). 5+ hosts running pre-2023 Chromium = stale-Chromium chained-CVE attack surface. Sequential-IP fleets visible (`147.135.103.70-75` 6-node Selenium cluster, `188.165.79.16-23` 4-node OVH cluster). | [browser-agent-cloud-survey-2026-05.md](browser-agent-cloud-survey-2026-05.md) |
| **AI safety / red-team self-hosted** | **5 cross-cloud (snapshot)** | Smallest population in the 2026-05 series. 4 DeepEval / Confident AI + **1 Garak (NVIDIA)** at `149.56.22.24:5000` — Garak is normally CLI-only, network-exposed deployment is novel. Zero Promptfoo / LangSmith confirmed; CLI-dominant deployment + commercial-tier auth-on + lack of cheap-VPS interest combine to make this category thin. | [ai-safety-eval-cloud-survey-2026-05.md](ai-safety-eval-cloud-survey-2026-05.md) |
