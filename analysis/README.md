...
**To write a new analysis:** copy [`_TEMPLATE`](_TEMPLATE) to `YYYY-MM-DD-sNN-slug` (e.g. `2026-05-22-s31-langfuse-cert-pivot`), fill in all 9 sections, add a row to the table below, commit + push.

---

| Date | File | Targets | Key Findings |
|---|---|---|---|
| 2026-05-22 | [2026-05-22-s31-llmops-observability](2026-05-22-s31-llmops-observability) | Evidently ML Monitoring (fingerprint); Agenta (carry-forward); Langfuse :5432 / Opik / PromptLayer (dispatched) | Evidently Tier-A no-auth default confirmed via Docker probe; aimap v1.9.24 shipped; Agenta open-signup verified across 6/6 hosts |
| 2026-05-22 | [2026-05-22-s31-langfuse-cert-pivot](2026-05-22-s31-langfuse-cert-pivot) | Langfuse :5432 Postgres cert-pivot corpus (11 hosts) | 11/11 Postgres auth-enforced (SCRAM-SHA-256); cert pivot on 34.0.11.208 → agenthub.cygnusalpha.one (signup-open, prod + dev); production S3 bucket names in dev CSP; 2× HIGH; `fe_sendauth` ≠ open Postgres (Insight #16 extended to protocol layer) |
